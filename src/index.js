...
const createConfig = () => {
  const baseConfig = {
    ...
    production: {
      port: process.env.PROD_PORT || 4500,
      protocol: 'https',
      corsOptions: {
      ...
      httpsOptions: {
        key: fs.readFileSync(process.env.SSL_KEY_PATH || path.join(__dirname, '..', 'backend-key.pem')),
        cert: fs.readFileSync(process.env.SSL_CERT_PATH || path.join(__dirname, '..', 'backend-cert.pem')),
      }
    }
  };
